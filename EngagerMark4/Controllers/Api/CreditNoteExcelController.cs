...
{
    public class CreditNoteExcelController : Controller
    {
        ICreditNoteRepository _repository;
        IWorkOrderService _workOrderService;

        public CreditNoteExcelController(ICreditNoteRepository repository, IWorkOrderService workOrderService)
        {
        ...
        public async Task<ActionResult> Download(Int64[] invoices)
        {
            if (invoices == null || invoices.Count() == 0)
            {
                ...
                {
                    var creditNoteIdsTemp = workOrders.Where(x => x.CreditNoteId.HasValue).Select(x => x.CreditNoteId.Value).ToList();

                    invoices = new long[creditNoteIdsTemp.Count()];
                    for (int i = 0; i < creditNoteIdsTemp.Count(); i++)
                    {
                        if (creditNoteIdsTemp[i] != 0)
                        {
                            invoices[i] = creditNoteIdsTemp[i];
                        }
                    }
                }
            }
            ...
            ExcelProcessor<CreditNoteExcel> processor = new ExcelProcessor<CreditNoteExcel>();



            string path = processor.ExportToExcel(_repository.GetData(invoices), Server.MapPath(FileConfig.EXPORT_EXCEL_CRNS), KeyUtil.GenerateKey());

            return base.File(path, "content-disposition", "Ez_CreditNote.xls");
        }
    }
}